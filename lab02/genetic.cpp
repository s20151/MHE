...
population_t genetic_algorithm(
        ...
        std::function<double(std::vector<bool>, std::vector<std::vector<int>>)> fitnes_f,
        ...
        })
{
    ...
    for (auto& gene : population)
        population_fit.push_back(fitnes_f(gene, point));
    ...
    auto replace_elite = [&](genotype_t e) {
        ...
        population_fit[worse_i] = fitnes_f(e, point);
    };
    ...
    while (termination_cond(population, population_fit, iteration)) {
        ...
        for (int i = 0; i < population.size(); i++)
            population_fit[i] = fitnes_f(population[i], point);
